...
static void GUI_Server(void *pvParameters)
{
	...
	DrawFont16x8("Mag:", 160, 9, WHITE);
	DrawFont16x8("Rc:x", 160, 62, WHITE);
	...
	while (1)
	{
		...
		//清空数据区
		ImageFill(
			(void *)&ImgBuffer[176 * 240 + 17], ImgFormat_RGB565, 240 - 8 * 30,
			MAGENTA,
			2 * 16, 8 * 30);
		if (xSemaphoreTake(openMVMutex, portMAX_DELAY))
        {
					sprintf(str, "dev_X:%f", OpenMV.dev_x);
					DrawFont16x8(str, 176, 90, BLUE);
					sprintf(str, "dev_Y:%f", OpenMV.dev_y);
					DrawFont16x8(str, 190, 90, BLUE);
          xSemaphoreGive(openMVMutex);
        }
		...
		//清空数据区
		ImageFill(
			(void *)&ImgBuffer[208 * 240 + 17], ImgFormat_RGB565, 240 - 8 * 30,
			MAGENTA,
			2 * 16, 8 * 30);
		...
		for (uint8_t i = 0; i < 8; ++i)
		{
			...
			if (GetPositionSensor(i, &sensor))
			{
				if (sensor.sensor_type == 2)
				{
					...
					sprintf(str, "%f", sensor.position.z);
					DrawFont16x8(str, 208+ i * 8, 110, BLUE);
				}
